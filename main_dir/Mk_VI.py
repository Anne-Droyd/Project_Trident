...
from main_dir.Methods.MDN_working import MDN
from main_dir.Methods.MDN import MDNLoss
...
def main():
    ...
    baumeister_data=data_ops.get_data()
    # data = data[data["density"].between(0.29,3.1)]
    ...
    epochs=1000
    batch_size=126
    output_weights=[1,1,1,1,1]
    y_col = ["m_core","zatm0","zdeep0","zatm1","zdeep1"]
    x_col = ["mass","req","Teq"]
    ...
    loss_func = MDNLoss(output_dim, N_MIXES,output_weights)
    ...
    mu_unscaled = np.exp(mu_unscaled).reshape(-1, N_MIXES, output_dim)
    ...
    sigma_unscaled = sigma * scale.reshape(1, 1, -1)
    # sigma = np.clip(sigma, 1e-3, None)

    # # Check if pi is collapsing
    # plt.hist(np.argmax(pi, axis=1), bins=N_MIXES)
    # plt.title("Most Likely Component Index Distribution")
    # plt.show()
    #
    # # Check variation in mu
    # mu_var = np.var(mu_unscaled[:, :, 0], axis=1)  # for m_core
    # plt.hist(mu_var, bins=100)
    # plt.title("Variance in Predicted mu for m_core")
    # plt.show()

    # # Choose a specific sample index (e.g., the first sample)
    # sample_idx = 0  # You can change this to visualize other samples
    #
    # # Get the pi values for this sample (shape should be (N_MIXES,))
    # pi_sample = pi[sample_idx]
    # # Plot the distribution of pi values for the chosen sample
    # plt.figure(figsize=(8, 6))
    # plt.bar(range(len(pi_sample)), pi_sample, alpha=0.7, color='b', label="Mixture Weights (pi)")
    # plt.xlabel('Mixture Component Index')
    # plt.ylabel('Probability (pi)')
    # plt.title(f"Distribution of Gaussian Weights (pi) for Sample {sample_idx}")
    # plt.legend()
    # plt.grid(True)
    # plt.show()
    ...
    # Get the index of the most likely component per sample
    # max_idx = np.argmax(pi, axis=1)  # shape: (batch_size,)
    #
    # sorted_indices = np.argsort(pi, axis=1)  # shape: (batch_size, N_MIXES)
    #
    # # Grab the index of the second highest pi value (second-to-last in sorted array)
    # second_max_idx = sorted_indices[:, -2]

    # Gather the corresponding μ from mu_unscaled
    # Output: shape (batch_size, output_dim)
    # mean_pred = np.array([mu_unscaled[i, idx] for i, idx in enumerate(max_idx)])
    samples = sample_from_mixture(pi, mu_unscaled, sigma_unscaled, n_samples=200)

    print(samples)
    mean_pred = np.mean(samples, axis=1)

    print(mean_pred)
    # plt.hist(mu_unscaled[:, :, output_dim].flatten(), bins=100)
    # plt.title("Predicted mu values")
    # plt.show()
    #
    # plt.hist(sigma[:, :, output_dim].flatten(), bins=100)
    # plt.title("Predicted sigma values")
    # plt.show()
    ...
    plots.plot_hist_pred_mean_vs_real(test_y,singular_prediction,"zdeep0")
    plots.plot_mdn_prediction(pi,mu_unscaled,sigma_unscaled)
    ...
    for i, name in enumerate(y_col):
        ...
        plt.scatter(y_true_unscaled.iloc[:, i], mean_pred[:, i],c="b", alpha=0.3, label="Predictions")
        plt.scatter([y_true_unscaled.iloc[:, i]],[y_true_unscaled.iloc[:, i]],c='r', label="Real")
